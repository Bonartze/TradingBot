...
os.makedirs(f"Data_zips/{time_interval}/{crypto_pair}/{year}", exist_ok=True)
os.makedirs(f"Data_extracted/{time_interval}/{crypto_pair}/{year}", exist_ok=True)
os.makedirs(f"StatisticsPlots/{crypto_pair}", exist_ok=True)
os.makedirs(f"StatisticsPlots/{crypto_pair}/{time_interval}/{year}", exist_ok=True)

months = [f'{year}-{month:02d}' for month in range(1, 13)]

for month in months:
    zip_file_path = f"Data_zips/{time_interval}/{crypto_pair}/{year}/{crypto_pair}-{time_interval}-{month}.zip"
    csv_file_path = f"Data_extracted/{time_interval}/{crypto_pair}/{year}/{crypto_pair}-{time_interval}-{month}.csv"
    ...
    try:
        ...
        with zipfile.ZipFile(zip_file_path, 'r') as zip_ref:
            zip_ref.extractall(f'./Data_extracted/{time_interval}/{crypto_pair}/{year}')
