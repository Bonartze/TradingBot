...
TradingStrategy::TradingStrategy(const TradingParams &trading_params_, double balance_, bool position_open_,
                                 double entry_price_, double asset_quantity_,  const std::string& symbol_ ,const std::string &key, const std::string &secret)
    : trading_params(trading_params_), balance(balance_), position_open(position_open_),
      entry_price(entry_price_), asset_quantity(asset_quantity_), symbol(symbol_) {
