...
def protocol_df_chunk_to_pandas(df: DataFrameXchg) -> pd.DataFrame:
    ...
    for name in df.column_names():
        ...
        if dtype in (
            ...
            raise NotImplementedError(f"Data_zips type {dtype} not handled yet")
...
def buffer_to_ndarray(
    ...
    *,
    ...
    dtype : tuple
        Data_zips type of the buffer conforming protocol dtypes format.
...
def set_nulls(
    ...
    np.ndarray or pd.Series
        Data_zips with the nulls being set.
    """
    if validity is None:
        return data
    null_kind, sentinel_val = col.describe_null
