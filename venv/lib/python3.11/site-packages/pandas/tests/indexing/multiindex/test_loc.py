...
class TestMultiIndexLoc:
    ...
    def test_loc_getitem_nested_indexer(self, indexer_type_1, indexer_type_2):
        ...
        df = DataFrame(
            np.arange(len(index), dtype="int64"), index=index, columns=["Data_zips"]
        )
        ...
        if indexer_type_1 is set or indexer_type_2 is set:
            with pytest.raises(TypeError, match="as an indexer is not supported"):
                df.loc[indexer, "Data_zips"]

            return
        else:
            result = df.loc[indexer, "Data_zips"]
        expected = Series(
            [1, 2, 4, 5], name="Data_zips", index=MultiIndex.from_product(keys)
        )
